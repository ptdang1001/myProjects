...
def main(opt):
    ...
    runPams["minusMean"] = 0
    runPams["xn"] = 20
    runPams["stdBias"] = 5/10
    runPams["consisThreshold"] = 0
    '''
    # l1 bases
    olabel, samples, colFeatures, optColFeatures = myUtils.getSpeBaseL1_small.main(runPams)
    '''
    ...
    # samples networks
    baseLen = 7
    sfcn = myModules.FCN(rowLen=baseLen, colLen=baseLen)
    sfcn = sfcn.to(device)
    ...
    rowLen = optColFeatures.size()[2]
    colLen = optColFeatures.size()[3]
    ofcn = myModules.FCN(rowLen=rowLen, colLen=colLen)
    ofcn = ofcn.to(device)
    ooptimizer = torch.optim.Adam(ofcn.parameters(), lr=opt.lr)
    ...
    # optColFeature data
    ores, oytrue_ypred = myUtils.myTrainTest.train_test(olabel, optColFeatures, ofcn, device,
    ...
    filePath = "/N/project/zhangclab/pengtao/myProjectsDataRes/20200113Predicte/results/l1SpeBaseTest/block1_small/excelRes/" + timeStam + ".xlsx"
