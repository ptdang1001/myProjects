...
def main(runPams):
    ...
    colFeature_basesConsisScores_conThreshold = [[f, basesConsisScores, consisThreshold] for f in colFeatures]
    optColFeatures = list(map(myUtils.myData.getOptFeature, colFeature_basesConsisScores_conThreshold))
    ...
    optColFeatures = optColFeatures.view(
        optColFeatures.size()[0],
        1,
    ...
    _, colFeatures = myUtils.myData.addNumMeanNoise(
        colFeatures, labels, int(
            len(colFeatures) / 3), mean, stdBias)
