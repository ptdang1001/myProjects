...
def train_test(label, data, Net, device, optimizer, lossFunc, opt):
    trainData, trainLabel, testData, testLabel = myUtils.myData.separateData(
        label, data, sep=5)
    # -------------------------------train-------------------------------------
    dataSet = myUtils.myData.GeneDataset(trainData, trainLabel)
    ...
    #myutils.mydraw.draw(LossList, opt.n_epochs)
            '''
    # ----------------------------------------------------------test-----------------------------------------
    dataSet = myUtils.myData.GeneDataset(testData, testLabel)
    dataLoader = DataLoader(dataset=dataSet,
                            batch_size=opt.batch_size,
                            shuffle=True,
                            num_workers=opt.n_cpu,
    ...
    for (x, y) in dataLoader:
        ...
        F1 += myUtils.myEvaluation.get_F1(b_y, predicted)
        JS += myUtils.myEvaluation.get_JS(b_y, predicted)
